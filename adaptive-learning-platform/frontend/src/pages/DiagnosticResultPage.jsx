  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div className="card-body">
                  ...
                  <ul style={{ paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {analysis.strengths.map((s, i) => (
                      <li key={i} style={{ color: 'var(--text-muted)', fontSize: '.875rem' }}>{s}</li>
                    ))}
                  </ul>
                </div>
              </div>
            ...
            {analysis.weaknesses?.length > 0 && (
              <div className="card" style={{ borderLeft: '3px solid #ef4444' }}>
                <div className="card-body">
                  <h4 style={{ fontWeight: 700, color: '#ef4444', marginBottom: 10 }}>A Melhorar</h4>
                  <ul style={{ paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {analysis.weaknesses.map((w, i) => (
                      <li key={i} style={{ color: 'var(--text-muted)', fontSize: '.875rem' }}>{w}</li>
                    ))}
                  </ul>
                </div>
              </div>
              ...
              <div key={i} className="card" style={{ borderLeft: `3px solid ${a.is_correct ? '#10b981' : '#ef4444'}` }}>
                <div className="card-body" style={{ padding: '14px 16px' }}>
                  ...
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', paddingLeft: 26 }}>
                    ...
                    {a.explanation && (
                      <p style={{ marginTop: 4 }}>💡 {a.explanation}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <button className="btn btn-primary btn-lg" onClick={() => navigate('/study-plan')}>
          Ver Plano de Estudo
        </button>
        <button className="btn btn-secondary btn-lg" onClick={() => navigate('/tutor')}>
          Tutor IA
        </button>
