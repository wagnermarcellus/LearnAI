...
exports.generateTest = async (req, res, next) => {
  try {
    ...
    // Persiste as questões (sem retornar gabarito ao aluno)
    const questions = [];
    for (let i = 0; i < aiResponse.questions.length; i++) {
      ...
      questions.push(qResult.rows[0]); // correct_option NÃO é retornada
    }
...
exports.submitAnswers = async (req, res, next) => {
  try {
    const { test_id, answers } = req.body; // answers: [{question_id, selected_option}]
    const userId = req.user.id;
    ...
    // Busca nomes dos tópicos para análise
