...
export default function DiagnosticTestPage() {
  ...
  const submit = async () => {
    setSubmitting(true)
    try {
      const answersArray = Object.entries(answers).map(([question_id, selected_option]) => ({ question_id, selected_option }))
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 16 }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontWeight: 500 }}>🤖 Gerando avaliação personalizada com IA…</p>
    </div>
  )

  if (errMsg) return <div className="alert alert-error" style={{ maxWidth: 500, margin: '40px auto' }}>{errMsg}</div>
    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-body">
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
            <p style={{ fontSize: '1.05rem', fontWeight: 600, lineHeight: 1.7, flex: 1 }}>{q.question_text}</p>
            <span className="badge" style={{ flexShrink: 0, background: diffColor[q.difficulty] + '20', color: diffColor[q.difficulty] }}>
              {q.difficulty}
            </span>
          </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
        ...
        {current < questions.length - 1 ? (
          <button className="btn btn-primary" onClick={() => setCurrent(c => c + 1)} disabled={!answers[q.id]}>
            Próxima →
          </button>
        ) : (
          <button
