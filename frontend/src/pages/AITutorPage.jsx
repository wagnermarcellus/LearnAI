...
function Message({ msg }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: 12 }}>
      {!isUser && (
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: 8, flexShrink: 0, fontSize: '1rem' }}>
          🤖
        </div>
      )}
      <div style={{
      ...
      {isUser && (
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: 8, flexShrink: 0, color: 'white', fontWeight: 700, fontSize: '.85rem' }}>
          EU
        </div>
      )}
    </div>
...
export default function AITutorPage() {
  ...
  useEffect(() => {
    api.get('/learning-paths/my-enrollments').then(r => setPaths(r.data.data || [])).catch(() => {})
  }, [])
  ...
  const send = async (text) => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/ai/chat', { message: msg, learning_path_id: pathId || undefined })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      ...
      <div className="page-header" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p className="page-subtitle">Explicações adaptadas ao seu nível · Contexto da trilha selecionada</p>
          </div>
          <select className="form-control" value={pathId} onChange={e => setPathId(e.target.value)} style={{ width: 'auto', minWidth: 220 }}>
          ...
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 5 }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--primary)', animation: `bounce .9s ${i * .15}s infinite` }} />
                ))}
              </div>
            </div>
          )}
      ...
      <style>{`@keyframes bounce { 0%,80%,100%{transform:scale(.7);opacity:.5} 40%{transform:scale(1);opacity:1} }`}</style>
    </div>
  )
}
