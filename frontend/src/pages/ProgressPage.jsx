...
export default function ProgressPage() {
  ...
  useEffect(() => {
    api.get('/progress/overview').then(r => setData(r.data.data)).finally(() => setLoading(false))
  }, [])
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ maxWidth: 260 }}>
            ...
            <div style={{ fontSize: '.72rem', opacity: .75, marginTop: 4 }}>{100 - xpPct} XP para o próximo nível</div>
          </div>
        </div>
        <div>
          <div style={{ fontSize: '.8rem', opacity: .8, marginBottom: 6 }}>Solicitações respondidas ao tutor</div>
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          <div className="card-body">
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <select className="form-control" value={pathId} onChange={e => setPathId(e.target.value)} style={{ flex: 1, minWidth: 200 }}>
                <option value="">Selecione uma trilha…</option>
                {data.enrollments.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}
              </select>
              <button className="btn btn-primary" onClick={requestTest} disabled={!pathId || requesting}>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Trilha', 'Tipo', 'Nota', 'Nível', 'Data'].map(h => (
                    <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '.78rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontWeight: 700, color: levelColor[t.level_assigned] }}>{parseFloat(t.score).toFixed(1)}%</span>
            ...
            {data.enrollments.map((e, i) => (
              <div key={i} className="card">
                <div className="card-body">
                  ...
                  {e.completed_at && <span className="badge badge-success" style={{ marginTop: 8 }}>✅ Concluída</span>}
                </div>
              </div>
            ))}
          </div>
            ...
            {data.badges.map(b => (
              <div key={b.id} className="card">
                <div className="card-body" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '.875rem' }}>{b.badge.replace(/_/g, ' ')}</div>
