...
export default function StudyPlanPage() {
  ...
  const generate = async () => {
    ...
    try {
      const res = await api.post('/study-plan/generate', { learning_path_id: pathId, goals: goals || undefined })
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Planos personalizados gerados pela IA com base no seu diagnóstico</p>
      </div>
        ...
        <div className="card-body">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              <input className="form-control" placeholder="Ex: Conseguir emprego em 6 meses" value={goals} onChange={e => setGoals(e.target.value)} />
            </div>
          </div>
          <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', marginBottom: 12 }}>
            ⚠️ É necessário ter concluído uma avaliação diagnóstica na trilha selecionada.
          </div>
          <button className="btn btn-primary" onClick={generate} disabled={!pathId || generating}>
            {generating ? <><div className="spinner" /> Gerando com IA…</> : '🤖 Gerar Plano Personalizado'}
          </button>
        </div>
      </div>
      ...
      {plans.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Complete uma avaliação diagnóstica e gere seu plano personalizado acima.</p>
        </div>
      ) : plans.map(plan => {
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <span className="badge badge-primary">⏱ {content?.estimated_weeks} semanas</span>
                    <span className="badge badge-success">📅 {content?.weekly_hours}h / semana</span>
              ...
              {content?.phases?.map(phase => (
                <div key={phase.phase} style={{ marginBottom: 14, padding: 16, background: 'var(--bg)', borderRadius: 10, border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--primary)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '.8rem', flexShrink: 0 }}>
                      {phase.phase}
                    </div>
